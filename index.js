...
const searchStock = async (url, portfolioUpdatedDate) => {
  ...
  const investorStock = await page.evaluate(() => {
    const stockList = Array.from(document.querySelectorAll('.stock')).map((li) => {
      const stockName = li.querySelector('a').innerText;
      const stockLink = li.querySelector('a').href;
      const portcentagePortfolio = li.nextSibling.nextElementSibling.innerText
      const recentActivity = li.nextSibling.nextElementSibling.nextElementSibling.innerText
  ...
  if(investorStock.length) {
    const inverstor = {
      investorName: await page.evaluate(() => document.querySelector('#f_name').innerText),
...
const listInvestor = async() => {
  ...
  const investorList = await page.evaluate(() => {
    const body = Array.from(document.querySelectorAll('#port_body ul li')).map((li) => {
      const name = li.querySelector('a').innerText;
      const url = li.querySelector('a').href;
      const portfolioUpdatedDate = li.querySelector('a').innerText?.split('Updated')[1]
